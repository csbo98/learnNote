...
/* Type to use for aligned memory operations.
   ...
   and store.  
   The 64-bit machine can load or store 8 byte in one instruction. 
              */
...
# define PAGE_COPY_FWD_MAYBE(dstp, srcp, nbytes_left, nbytes)		      \
  do									      \
    {									      \
    // 字节数大于指定的阈值，并且源地址和目的地址在各自页内的偏移量相同才能使用操纵虚拟地址的方法做页拷贝
      if ((nbytes) >= PAGE_COPY_THRESHOLD				      \
	  && PAGE_OFFSET ((dstp) - (srcp)) == 0)			      \ // (a - b) % c = ((a % c) - (b % c)) % c
	{								      \
	  /* The amount to copy is past the threshold for copying	      \
	     pages virtually with kernel VM operations, and the		      \
	     source and destination addresses have the same alignment.  */    \
	  // 算出在这一页内有多少需要拷贝的字节数，先执行word拷贝，把dstp和srcp都指向下一页的起始地址
    size_t nbytes_before = PAGE_OFFSET (-(dstp));			      \
	  if (nbytes_before != 0)					      \
	    {								      \
      // 前面已经对齐了，在这里就可以使用word拷贝了
