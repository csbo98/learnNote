...
右值引用对移动语义的帮助是：可以通过右值引用和左值引用区分是需要调用拷贝构造函数还是移动构造函数。如果通过
std::move()把一个对象传递给它的移动构造函数之后，那么就不能直接访问这个对象的值，可能是未定义的结果。传递右值引用能不能减小代价取决于移动构造函数如何实现，如果移动构造函数定义同拷贝构造函数相似的实现，那么传递右值引用和传递左值引用的代价是一样的。       
